...
def test_build_default_processor_registry() -> None:
    ...
    assert len(registry.processors) == 13
    ...
    assert isinstance(
        registry.processors[5], WellsFargoBusinessCheckingProcessor
    )
    assert isinstance(
        registry.processors[6], WellsFargoBusinessCreditCardProcessor
    )
    assert isinstance(
        registry.processors[7], WellsFargoBusinessLineOfCreditProcessor
    )
    assert isinstance(
        registry.processors[8], AmericanExpressCreditCardProcessor
    )
    assert isinstance(
        registry.processors[9], AmericanExpressBusinessCheckingProcessor
    )
    assert isinstance(
    ...
    assert isinstance(
        registry.processors[11], AmericanExpressPersonalLoanProcessor
    )
