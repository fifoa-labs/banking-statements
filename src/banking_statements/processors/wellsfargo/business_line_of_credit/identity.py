"""
src/banking_statements/processors/wellsfargo/business_line_of_credit/identity.py

Identity parsing for supported Wells Fargo business line-of-credit statements.
"""
...
class WellsFargoBusinessLineOfCreditIdentity:
    """Identity fields parsed from a Wells Fargo
    business line-of-credit statement.
    """
...
def _parse_date(value: str) -> date:
    """Parse a Wells Fargo business line-of-credit closing date."""
    return datetime.strptime(value, "%m/%d/%y").date()  # noqa: DTZ007


def parse_identity(
    text: StatementText,
) -> WellsFargoBusinessLineOfCreditIdentity:
    ...
    if account_match is None:
        msg = (
            "Wells Fargo business line-of-credit account number was not found."
        )
    ...
    if ending_matches and ending_matches != {last4}:
        msg = (
            "Wells Fargo business line-of-credit account numbers do not agree."
        )
        raise ValueError(msg)

    closing_match = _CLOSING_DATE_PATTERN.search(full_text)
    if closing_match is None:
        msg = "Wells Fargo business line-of-credit closing date was not found."
        raise ValueError(msg)

    billing_days_match = _BILLING_DAYS_PATTERN.search(full_text)
    if billing_days_match is None:
        msg = (
            "Wells Fargo business line-of-credit billing cycle was not found."
        )
        raise ValueError(msg)
    ...
    return WellsFargoBusinessLineOfCreditIdentity(
        account=AccountIdentity(
