"""
src/banking_statements/processors/wellsfargo/credit_card/processor.py

Statement processor for the first supported Wells Fargo credit-card format.
"""
...
class WellsFargoCreditCardProcessor:
    """Processor for supported Wells Fargo credit-card statements."""

    @property
    def name(self) -> str:
        """Return the stable processor identifier."""
        return "wellsfargo.credit_card.v1"
    ...
    def match(self, text: StatementText) -> ProcessorMatch:
        ...
        matched = any(
            ...
            for signature in WELLS_FARGO_CREDIT_CARD_SIGNATURES
        )
        ...
        return ProcessorMatch(
            ...
            reason=(
                ...
                if matched
                else "Required Wells Fargo credit-card markers were not found."
            ),
        )
    ...
    def parse(
        ...
        """Parse a Wells Fargo credit-card statement."""
